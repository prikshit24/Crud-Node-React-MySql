...
export const Networking = (props) => {
    const { tableName, showTable, tableData, networkingSave } = props
        ...
        <>
            {(tableName === 'networkingScore' && showTable === true) && (
                <Box sx={{ display: 'flex', border: '0.5px solid #b8b8b8' }}>
                    ...
                    <Box p={1} pl={2} pr={2} width={'20%'} sx={{ borderRight: '0.5px solid white' }}>
                        <Typography variant='body1'>Not Present</Typography>
                    </Box>
                    <Box width={'20%'} p={1} pl={2} pr={2}></Box>
                </Box>
            )}
            {
                tableData.map((data, i) => {
                    if (tableName === 'networkingScore' && showTable === true) {
                        return (
                            <Box key={i} sx={{ display: 'flex', border: '0.5px solid #b8b8b8' }}>
                                <Box p={1} pl={2} pr={2} width={'40%'} sx={{ borderRight: '0.5px solid black' }}>
                                    <Typography variant='body1'>{data.name}</Typography>
                                </Box>
                                <Box p={1} pl={2} pr={2} width={'20%'} sx={{ borderRight: '0.5px solid black', display: 'flex', gap: 1 }}>
                            ...
                            if (tableName === 'networkingScore' && showTable === true) {
                                return (
                                    <Box key={i} sx={{ display: 'flex', border: '0.5px solid #b8b8b8' }}>
                                        ...
                                        <Box p={1} pl={2} pr={2} width={'25%'} sx={{ borderRight: '0.5px solid black', display: 'flex' }}>
                                            <Typography variant='body1'>{data.present}</Typography>
                                        </Box>
                                        <Box p={1} pl={2} pr={2} width={'25%'} sx={{display: 'flex', justifyContent:'space-between' }}>
                                            <Typography variant='body1'>{data.notPresent}</Typography>
