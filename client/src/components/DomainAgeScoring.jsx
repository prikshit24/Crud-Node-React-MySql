...
export const DomainAgeScoring = ({ dSave, domainAgeState }) => {

  const [state, setState] = useState({
    name:'', best: 0, average: 0, good: 0, bad: 0, worse: 0, tableName:'DomainAgeScore'
  })
  ...
  const showData = () => {
    if((state.name !== '') && (!domainAgeState.map((data) => data.name).includes(state.name))){
      dSave((item) => [...item,state])
    }
  }

  return (
    <Box my={2}>
      <Typography variant='h6'>Domain Age Scoring</Typography>
