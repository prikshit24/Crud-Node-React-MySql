...
export const KeywordFactorScoring = ({ kSave, keywordFactorState }) => {

  const [state, setState] = useState({
    name: '', present: 0, notPresent: 0, good: 0, average:0, bad: 0, tableName:'KeywordFactorScore'
  })
  ...
  const showData = () => {
    if((state.name !== '') && (!keywordFactorState.map((data) => data.name).includes(state.name))){
    kSave((item) => [...item,state])
    }
  }

  return (
    <Box my={2}>
      <Typography variant='h6'>KeyWord Factor Scoring</Typography>
