...
export const ImageSEOScoring = ({ iSave, imageSEOState }) => {

  const [state, setState] = useState({
    name: '', best: 0, average: 0, good: 0, bad: 0, worse: 0, tableName:'ImageSEOScore'
  })
  ...
  const showData = () => {
    if((state.name !== '') && (!imageSEOState.map((data) => data.name).includes(state.name))){
    iSave((item) => [...item,state])
    }
  }

  return (
    <Box my={2}>
      <Typography variant='h6'>Image SEO Scoring</Typography>
